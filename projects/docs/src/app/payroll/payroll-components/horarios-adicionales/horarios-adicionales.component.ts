...
export class HorariosAdicionalesComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.dataSource$ = new BehaviorSubject<NgGtdDS>({
      datasource: new MatTableDataSource<HorarioAdicional>(HorariosAdicionales),
  ...
  add(name: HorarioAdicional): void {
    if (!name) {
      return;
    }
    let datasource = this.dataSource$.value.datasource;
    datasource.data = [
      ...
      {
        ...name,
        id: this.memSrv.genId(datasource.data, 'horariosAdicionales'),
      },
    ];
    this.dataSource$.next({
      ...this.dataSource$.value,
  ...
  delete(libranza: HorarioAdicional): void {
    let datasource = this.dataSource$.value.datasource;
  ...
  edit(libranza: HorarioAdicional): void {
    let datasource = this.dataSource$.value.datasource;
