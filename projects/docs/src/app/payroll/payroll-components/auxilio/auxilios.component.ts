...
export class AuxiliosComponent implements OnInit, AfterViewInit {
  ...
  add(name: Auxilio): void {
    ...
    datasource.data = [
      ...datasource.data,
      { ...name, id: this.memSrv.genId(datasource.data, 'auxilios') },
    ];
    this.dataSource$.next({
      ...this.dataSource$.value,
      datasource: datasource,
